...
function DataChip({ label, value, color = BRAND.espresso }: { label: string; value: string; color?: string }) {
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '20px 24px', borderRadius: 24, background: BRAND.white, border: `1px solid ${BRAND.espresso}08`, boxShadow: '0 10px 30px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: color }} />
                <div style={{ fontFamily: FONTS.main, fontSize: F_SIZE.sm, fontWeight: 800, color: BRAND.espresso, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{label}</div>
            </div>
            <div style={{ paddingLeft: 22, fontFamily: FONTS.main, fontSize: F_SIZE.md, fontWeight: 600, color: BRAND.espresso, lineHeight: 1.6, opacity: 0.9 }}>{value}</div>
        </div>
    );
}
...
function SectionHeader({ eyebrow, title, desc }: { eyebrow: string; title: React.ReactNode; desc: React.ReactNode }) {
    return (
        <div style={{ marginBottom: 40 }}>
            ...
            <h2 style={{ fontFamily: FONTS.main, fontSize: F_SIZE.xl, fontWeight: 900, color: BRAND.espresso, lineHeight: 1.15, letterSpacing: '-0.04em', margin: '0 0 20px' }}>
                {title}
            </h2>
            <p style={{ fontFamily: FONTS.main, fontSize: F_SIZE.md, color: BRAND.taupe, lineHeight: 1.8, margin: 0, fontWeight: 600 }}>
                {desc}
            </p>
        </div>
    );
...
export default function CanWeGetFromFood() {
    ...
    return (
        <section ref={sectionRef} style={{ background: BRAND.cream, padding: '100px 24px', position: 'relative', overflow: 'hidden' }}>
                ...
                <div className="food-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 1.2fr', gap: 64, alignItems: 'center' }}>
                    ...
                    <div>
                        <SectionHeader 
                            ...
                            title={<span>Can we get everything <br /> from <span style={{ color: BRAND.burgundy }}>food?</span></span>} 
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 40 }}>
                            <DataChip label="Iron Source" value="To meet daily iron needs, you would need around 600 grams of spinach every single day." color={BRAND.burgundy} />
                            <DataChip label="Calcium Source" value="To reach daily calcium targets, you would need at least 300 grams of ragi daily." color={BRAND.burgundy} />
                            <DataChip label="Vitamin D3 Source" value="To meet the requirement, you would need to consume 15+ eggs every day." color={BRAND.burgundy} />
                        </div>
                        ...
                        <motion.div
                            ...
                            style={{ padding: '32px', background: BRAND.white, borderRadius: 40, border: `1px solid ${BRAND.espresso}08`, boxShadow: '0 20px 40px rgba(0,0,0,0.02)' }}
                        >
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                                <Info size={18} color={BRAND.taupe} />
                                <div style={{ fontFamily: FONTS.main, fontSize: F_SIZE.sm, fontWeight: 900, textTransform: 'uppercase', color: BRAND.espresso, letterSpacing: '0.15em' }}>The Practical Reality</div>
                            </div>
                            <h4 style={{ fontFamily: FONTS.main, fontSize: F_SIZE.lg, fontWeight: 900, color: BRAND.espresso, marginBottom: 16 }}>Knowledge isn't the problem.</h4>
                            <p style={{ fontFamily: FONTS.main, fontSize: F_SIZE.md, color: BRAND.taupe, lineHeight: 1.8, margin: 0, fontWeight: 500 }}>
                                Even with complete nutritional knowledge, consistency remains the logistical hurdle. PlainFuel is designed to span this gap effortlessly.
                            </p>
