      ...
      <MainLayout background={BRAND.light}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '160px 24px 100px', textAlign: 'center', fontFamily: FONTS.main }}>
          ...
          >
            ...
            <h1 style={{ fontSize: F_SIZE.xl, fontWeight: 900, color: BRAND.primary, margin: '0 0 16px', letterSpacing: '-0.04em', lineHeight: 1 }}>
              Your <span style={{ fontWeight: 300, color: BRAND.secondary }}>Cart</span> is Empty
            </h1>

            <p style={{ fontSize: F_SIZE.lg, fontFamily: FONTS.accent, color: BRAND.secondary, fontWeight: 700, marginBottom: 48 }}>
            ...
            <motion.button
              ...
              style={{
                padding: '18px 36px', background: BRAND.primary, color: BRAND.white, border: 'none', borderRadius: 6,
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '160px 24px 100px', position: 'relative', zIndex: 1, fontFamily: FONTS.main }}>
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 20, marginTop: 24 }}>
            <h1 style={{ fontSize: F_SIZE.xl, fontWeight: 900, color: BRAND.primary, margin: 0, letterSpacing: '-0.04em', lineHeight: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 380px', gap: 40, alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ 
              ...
              <div style={{ padding: '28px 32px', borderBottom: `1px solid ${BRAND.tertiary}40` }}>
                  ...
                  {items.map((item, index) => (
                    <motion.div
                      ...
                      style={{
                        ...
                        paddingBottom: 28, marginBottom: 28, borderBottom: `1px solid ${BRAND.tertiary}40`
                      }}
                    >
                      {/* Product Image */}
                      <div style={{
                        ...
                        display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 10, border: `1px solid ${BRAND.tertiary}20`
                      }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                        <div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                             <h3 style={{ fontSize: F_SIZE.md, fontWeight: 900, color: BRAND.primary, margin: '0 0 6px' }}>{item.productName}</h3>
                             <motion.button 
                               whileHover={{ scale: 1.1, color: BRAND.primaryDark }}
                               onClick={() => handleRemoveItem(item.id!)}
                               style={{ background: 'none', border: 'none', color: BRAND.secondary, cursor: 'pointer', padding: 4 }}
                             >
                                <Trash2 size={18} />
                             </motion.button>
                          </div>
                          <p style={{ fontSize: F_SIZE.sm, color: BRAND.secondary, margin: '0 0 12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                            {item.packageName}
                          </p>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            <span style={{ fontSize: F_SIZE.md, fontWeight: 900, color: BRAND.primary }}>₹{item.price.toLocaleString()}</span>
                            {item.origPrice && item.origPrice > item.price && (
                              <span style={{ fontSize: F_SIZE.sm, color: BRAND.secondary, textDecoration: 'line-through' }}>₹{item.origPrice.toLocaleString()}</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginTop: 12 }}>
                          <div style={{ 
                            display: 'flex', alignItems: 'center', gap: 16, border: `1px solid ${BRAND.tertiary}60`, borderRadius: 4, padding: '4px 12px', background: BRAND.light
                          }}>
                            <button onClick={() => updateQuantity(item.id!, item.quantity - 1)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: BRAND.primary, fontWeight: 900, fontSize: 18 }}>−</button>
                            <span style={{ fontSize: F_SIZE.sm, fontWeight: 900, color: BRAND.primary, minWidth: 24, textAlign: 'center' }}>{item.quantity}</span>
                            <button onClick={() => updateQuantity(item.id!, item.quantity + 1)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: BRAND.primary, fontWeight: 900, fontSize: 18 }}>+</button>
                          </div>
                        </div>
                      </div>
          ...
          <div style={{ position: 'sticky', top: 120 }}>
            <div style={{ 
              background: BRAND.white, border: `1px solid ${BRAND.tertiary}40`, borderRadius: 16, 
              padding: '36px', boxShadow: '0 10px 30px -10px rgba(50,45,41,0.04)' 
            }}>
              <h3 style={{ fontSize: F_SIZE.lg, fontWeight: 900, color: BRAND.primary, margin: '0 0 24px', letterSpacing: '-0.01em' }}>Billing Details</h3>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                   <span style={{ fontSize: F_SIZE.sm, color: BRAND.secondary, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Cart Value</span>
                  <span style={{ fontWeight: 600, color: BRAND.primary }}>₹{totalPrice.toLocaleString()}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: F_SIZE.sm, color: BRAND.secondary, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Priority Shipping</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <span style={{ fontSize: F_SIZE.md, fontWeight: 900, color: BRAND.primary }}>Total Order Value</span>
                   <span style={{ fontSize: F_SIZE.lg, fontWeight: 900, color: BRAND.primary }}>₹{totalPrice.toLocaleString()}</span>
                </div>
                ...
                <motion.button
                  ...
                  style={{
                    marginTop: 24, padding: '20px', background: BRAND.primary, color: BRAND.white, border: 'none',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 24, padding: '20px', background: BRAND.light, borderRadius: 8 }}>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    <ShieldCheck size={16} color={BRAND.secondary} />
                    <span style={{ fontSize: 10, fontWeight: 800, color: BRAND.secondary, textTransform: 'uppercase', letterSpacing: '0.05em' }}>End-to-End SSL Encrypted</span>
                  </div>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    <Truck size={16} color={BRAND.secondary} />
                    <span style={{ fontSize: 10, fontWeight: 800, color: BRAND.secondary, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Express Delivery Verified</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {showToast && (
          <motion.div
            ...
            style={{
              ...
              background: BRAND.primary, color: BRAND.white, padding: '16px 32px', borderRadius: 100,
