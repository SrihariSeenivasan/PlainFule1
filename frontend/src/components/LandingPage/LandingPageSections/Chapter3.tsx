    ...
    <section style={{ padding: '52px 24px 0', maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(8px, 3vw, 20px)', flexWrap: 'nowrap' }}>
        <motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }}
          ...
          style={{ height: 1, width: 'clamp(24px, 10vw, 80px)', background: BRAND.primary, opacity: 0.18, transformOrigin: 'right', flexShrink: 0 }} />
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '6px 20px', borderRadius: 100, background: BRAND.light, border: `1px solid ${BRAND.tertiary}`, whiteSpace: 'nowrap' }}>
          <Sparkles size={13} color={BRAND.accent} />
          <span style={{ fontSize: 'clamp(0.8rem, 2.5vw, 1.1rem)', fontWeight: 800, color: BRAND.primary, letterSpacing: '0.18em', textTransform: 'uppercase' }}>Chapter {number}</span>
        </motion.div>
        <motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }}
          transition={{ duration: 0.7 }}
          style={{ height: 1, width: 'clamp(24px, 10vw, 80px)', background: BRAND.primary, opacity: 0.18, transformOrigin: 'left', flexShrink: 0 }} />
      </div>
    </section>
  );
}
...
function ProblemsSection() {
  ...
  return (
    <section ref={ref} style={{ padding: 'clamp(40px, 5vh, 60px) 24px', maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
      ...
      {/* Two column grid: Content + Video — collapses on mobile */}
      <div className="ch3-problems-grid">
        ...
        >
          <motion.h2
            ...
            style={{ fontSize: 'clamp(1.8rem, 3.5vw, 2.8rem)', fontWeight: 900, color: BRAND.primary, margin: '0 0 24px', letterSpacing: '-0.035em', lineHeight: 1.15 }}>
          ...
          <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.7, delay: 0.8 }}
            style={{ fontSize: F_SIZE.md, color: BRAND.secondary, lineHeight: 1.7, margin: 0, fontWeight: 800 }}>
            Maintaining a complex routine is a logistical challenge. Most people start with good intent but stop within days.{' '}
            <strong style={{ color: BRAND.primary }}>The real issue isnt effort — its that the current system is too complex for a busy life.</strong>
          </motion.p>
        </motion.div>
        ...
        >
          <div style={{ width: '100%', height: '100%', background: `linear-gradient(135deg, #f5f1ed 0%, #faf8f6 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
            ...
            <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
              style={{ width: 90, height: 90, borderRadius: '50%', background: BRAND.accent, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', position: 'relative', zIndex: 2, boxShadow: `0 20px 60px ${BRAND.accent}40` }}>
              <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
                <path d="M15 10L30 20L15 30V10Z" fill="white" />
              </svg>
            </motion.div>
            {[1, 2].map((i) => (
              <motion.div key={i} animate={{ scale: [1, 1.6], opacity: [1, 0] }} transition={{ duration: 2, repeat: Infinity, delay: i * 0.5 }}
                style={{ position: 'absolute', borderRadius: '50%', border: `2px solid ${BRAND.accent}`, width: 90 + i * 40, height: 90 + i * 40, pointerEvents: 'none' }} />
            ))}
          </div>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.5 }}
            style={{
...
function SolutionSection() {
  ...
  return (
    <section ref={ref} style={{ padding: 'clamp(40px, 5vh, 60px) 24px', maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
      ...
      {/* Two column grid: Image + Content — collapses on mobile */}
      <div className="ch3-solution-grid">

        {/* LEFT — Product image */}
        <motion.div
          initial={{ opacity: 0, scale: 0.92 }} animate={inView ? { opacity: 1, scale: 1 } : {}}
          transition={{ duration: 1, delay: 0.15, ease: [0.16, 1, 0.3, 1] }}
          className="ch3-solution-image"
          style={{ position: 'relative', borderRadius: 24, overflow: 'hidden', background: 'transparent', border: 'none', boxShadow: 'none' }}
        >
          <Image
          ...
          <motion.div animate={{ x: ['-100%', '100%'] }} transition={{ duration: 5, repeat: Infinity, ease: 'linear' }}
            style={{ position: 'absolute', inset: 0, background: `linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.15) 50%, transparent 100%)`, pointerEvents: 'none', zIndex: 1 }} />
        </motion.div>
        ...
        <motion.div
          ...
          style={{ position: 'relative', zIndex: 2 }}
        >
          <motion.div initial={{ scaleX: 0 }} animate={inView ? { scaleX: 1 } : {}} transition={{ duration: 0.7, delay: 0.2 }}
          ...
          <motion.h2 initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.3, ease: [0.16, 1, 0.3, 1] }}
            style={{ fontSize: 'clamp(1.8rem, 3.5vw, 2.8rem)', fontWeight: 900, color: BRAND.primary, margin: '0 0 12px', letterSpacing: '-0.035em', lineHeight: 1.15 }}>
          ...
          <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.7, delay: 0.4 }}
            ...
            {solutions.map((item, i) => {
              ...
              return (
                <motion.div key={i}
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 12, background: color.bg, border: `1.5px solid ${color.border}`, position: 'relative', overflow: 'hidden', cursor: 'pointer', transition: 'all 0.3s ease' }}>
                  ...
                  <div style={{ width: 30, height: 30, borderRadius: 6, background: color.badge, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative', zIndex: 1 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.8 }}
  ...
  return (
    ...
    >
      <button onClick={onToggle}
        style={{ width: '100%', background: 'none', border: 'none', cursor: 'pointer', padding: '28px 0', display: 'flex', alignItems: 'center', gap: 24, textAlign: 'left' }}>
        ...
        <span style={{ fontSize: F_SIZE.lg, fontWeight: 900, color: BRAND.primary, flex: 1, lineHeight: 1.2, letterSpacing: '-0.02em', textAlign: 'left' }}>
          {data.headline}
        </span>
        <motion.div animate={{ rotate: isOpen ? 45 : 0 }} transition={{ duration: 0.35 }}
      ...
      <motion.div initial={false} animate={{ height: isOpen ? 'auto' : 0, opacity: isOpen ? 1 : 0 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} style={{ overflow: 'hidden' }}>
        <div className="ch3-accordion-body">
          {data.contentBefore && (
            <p style={{ fontSize: F_SIZE.md, color: BRAND.secondary, margin: '0 0 24px', lineHeight: 1.7, fontWeight: 800 }}>{data.contentBefore}</p>
          )}
          {data.content && (
            <p style={{ fontSize: F_SIZE.md, color: BRAND.secondary, margin: '0 0 24px', lineHeight: 1.7, fontWeight: 800 }}>{data.content}</p>
          )}
          {/* List — 2-col on desktop, 1-col on mobile */}
          <div className="ch3-accordion-list">
  ...
  return (
    <section ref={ref} style={{ padding: '80px 0 0', position: 'relative', zIndex: 1 }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
        {/* Section header — 2-col on desktop, stacked on mobile */}
        <div className="ch3-logic-header">
          <div>
...
function SimpleProcessSection() {
  ...
  return (
    <section ref={ref} style={{ padding: '80px 24px 80px', maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
      ...
      {/* ── BENTO GRID ── */}
      <div className="ch3-bento-grid">

        {/* Tile 1 — Hero dark tile (spans 5 cols) */}
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={inView ? { opacity: 1, scale: 1 } : {}}
          transition={{ duration: 0.8, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
          className="ch3-bento-tile1"
          style={{ background: BRAND.primary, borderRadius: 28, padding: '36px 32px', position: 'relative', overflow: 'hidden' }}>
        ...
        {/* Tile 2 — "21 ingredients" accent tile */}
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.7, delay: 0.2 }}
          className="ch3-bento-tile2"
          style={{ background: BRAND.accent, borderRadius: 28, padding: '28px 24px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', overflow: 'hidden', position: 'relative' }}>
        ...
        {/* Tile 3 — "100%" light tile */}
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.7, delay: 0.25 }}
          className="ch3-bento-tile3"
          style={{ background: BRAND.light, borderRadius: 28, padding: '28px 24px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', border: `1px solid ${BRAND.tertiary}`, position: 'relative', overflow: 'hidden' }}>
        ...
        {/* Tile 4 — Ingredients table */}
        <motion.div initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : {}}
          transition={{ duration: 0.8, delay: 0.3, ease: [0.16, 1, 0.3, 1] }}
          className="ch3-bento-tile4"
          style={{ background: BRAND.white, borderRadius: 28, padding: '28px 28px', border: `1px solid ${BRAND.quaternary}` }}>
        ...
        {/* Tile 5 — "0 mg" + "≥65%" stacked */}
        <div className="ch3-bento-tile5" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <motion.div initial={{ opacity: 0, x: 20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.35 }}
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.45 }}
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          className="ch3-bento-tile6"
          style={{ background: BRAND.accent, borderRadius: 28, padding: '40px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 32, flexWrap: 'wrap', position: 'relative', overflow: 'hidden' }}>
          {[1, 2].map(r => (
  ...
  return (
    <div style={{ background: BRAND.white, overflow: 'hidden', position: 'relative' }}>
      ...
      <style>{`
        ...
        .ch3-bento-tile1 { grid-column: 1 / 6; grid-row: 1; }
        .ch3-bento-tile2 { grid-column: 6 / 9; grid-row: 1; }
        .ch3-bento-tile3 { grid-column: 9 / 13; grid-row: 1; }
        .ch3-bento-tile4 { grid-column: 1 / 8; grid-row: 2; }
        .ch3-bento-tile5 { grid-column: 8 / 13; grid-row: 2; }
        .ch3-bento-tile6 { grid-column: 1 / 13; grid-row: 3; }
        ...
        @media (max-width: 900px) {
          ...
          /* Bento: 2-col layout on tablet */
          .ch3-bento-grid {
            grid-template-columns: 1fr 1fr !important;
          }
          .ch3-bento-tile1 { grid-column: 1 / -1 !important; grid-row: auto !important; }
          .ch3-bento-tile2 { grid-column: 1 / 2 !important; grid-row: auto !important; }
          .ch3-bento-tile3 { grid-column: 2 / 3 !important; grid-row: auto !important; }
          .ch3-bento-tile4 { grid-column: 1 / -1 !important; grid-row: auto !important; }
          .ch3-bento-tile5 { grid-column: 1 / -1 !important; grid-row: auto !important; flex-direction: row !important; }
          .ch3-bento-tile6 { grid-column: 1 / -1 !important; grid-row: auto !important; }
        }
        ...
        @media (max-width: 600px) {
          ...
          .ch3-bento-tile2 { grid-column: 1 / -1 !important; }
          .ch3-bento-tile3 { grid-column: 1 / -1 !important; }
          .ch3-bento-tile5 { flex-direction: column !important; }
        }
      `}</style>
