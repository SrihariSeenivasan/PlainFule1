  ...
  return (
    <motion.div
      ...
      style={{
        ...
        fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 14, fontWeight: 500,
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 10 }}>
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} style={{ textAlign: 'center', marginBottom: 88 }}>
            ...
            <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.5 }}
              style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 18, color: '#666', maxWidth: 580, margin: '0 auto', lineHeight: 1.75 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {[
                ...
                <motion.div key={i}
                  ...
                  style={{ background: T.paper, border: `2.5px solid ${item.color}`, borderRadius: 16, padding: '14px 16px', boxShadow: `5px 5px 0 ${item.color}28`, transform: `rotate(${item.rotate}deg)`, display: 'flex', alignItems: 'flex-start', gap: 14, position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, flexShrink: 0 }}>
                    <div style={{ width: 32, height: 32, borderRadius: '50%', background: item.color, color: '#fff', fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `2px 2px 0 ${item.color}55` }}>{item.step}</div>
                    <span style={{ fontSize: 18 }}>{item.icon}</span>
                  </div>
                  <div>
                    <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 10, fontWeight: 700, color: item.color, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 2 }}>{item.label}</div>
                    <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 16, fontWeight: 600, color: T.ink, lineHeight: 1.2, marginBottom: 4 }}>{item.title}</div>
                    <InkUnderline color={item.color} width={80} />
                    <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 14, color: '#555', lineHeight: 1.55, marginTop: 4 }}>{item.desc}</div>
                  </div>
                </motion.div>
              ))}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {[
                ...
                <motion.div key={i}
                  initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}
                  whileHover={{ x: 5, scale: 1.02 }}
                  style={{ background: T.paper, border: `2.5px solid ${item.color}`, borderRadius: 16, padding: '14px 16px', boxShadow: `5px 5px 0 ${item.color}28`, transform: `rotate(${item.rotate}deg)`, display: 'flex', alignItems: 'center', gap: 14, position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 4, background: item.color, borderRadius: '16px 0 0 16px' }} />
                  ...
                  <div>
                    <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 30, fontWeight: 700, color: item.color, lineHeight: 1 }}>{item.stat}</div>
                    <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 14, color: '#555', lineHeight: 1.4 }}>{item.label}</div>
                  </div>
                </motion.div>
              ))}
          ...
          <div className="ch3-grid" style={{ marginBottom: 20 }}>
            ...
            {/* LEFT COLUMN: 5 gap cards */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 11, fontWeight: 700, color: T.blue, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4, paddingLeft: 4 }}>
                — The 5 Gaps
              </div>
              {gapItems.map((item, i) => (
                <motion.div key={i}
                  ...
                  style={{ background: T.paper, border: `2px solid ${item.color}`, borderRadius: 14, padding: '11px 13px 11px 16px', boxShadow: `4px 4px 0 ${item.color}22`, transform: `rotate(${item.rotate}deg)`, display: 'flex', alignItems: 'flex-start', gap: 10, position: 'relative', overflow: 'hidden', cursor: 'pointer' }}>
                  <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 4, background: item.color, borderRadius: '14px 0 0 14px' }} />
                  <div style={{ flexShrink: 0, width: 32, height: 32, borderRadius: '50%', background: `${item.color}18`, border: `2px solid ${item.color}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>{item.icon}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 1 }}>
                      <span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 13, fontWeight: 600, color: T.ink }}>{item.title}</span>
                      <span style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 9, fontWeight: 700, color: '#fff', background: item.color, padding: '1px 6px', borderRadius: 8 }}>{item.badge}</span>
                    </div>
                    <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 11, fontWeight: 600, color: item.color, marginBottom: 2 }}>{item.values}</div>
                    <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 12, color: '#666', lineHeight: 1.45 }}>{item.desc}</div>
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {statItems.map((stat, i) => (
                <motion.div key={i}
                  ...
                  whileHover={{ x: -4, scale: 1.02 }}
                  style={{ background: T.paper, border: `2px solid ${stat.color}`, borderRadius: 14, padding: '12px 14px', boxShadow: `4px 4px 0 ${stat.color}22`, transform: `rotate(${stat.rotate}deg)`, display: 'flex', alignItems: 'center', gap: 12, position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 4, background: stat.color, borderRadius: '14px 0 0 14px' }} />
                  <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 34, fontWeight: 700, color: stat.color, lineHeight: 1, flexShrink: 0, minWidth: 64, textAlign: 'center' }}>{stat.num}</div>
                  <div>
                    <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 13, color: T.ink, fontWeight: 600, lineHeight: 1.35, marginBottom: 2 }}>{stat.label}</div>
                    <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", fontSize: 11, color: '#999', fontStyle: 'italic' }}>{stat.sublabel}</div>
                    <div style={{ marginTop: 4 }}><InkUnderline color={stat.color} width={50} /></div>
                  </div>
                </motion.div>
              ))}
