      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '120px 24px 80px' }}>
        ...
        <motion.button
          ...
          style={{
            ...
            color: C.forest, fontWeight: 900, fontSize: F_SIZE.sm,
        ...
        <div style={{ 
              ...
              {images.length > 1 && (
                <>
                  <button onClick={handlePrevImage} style={{ position: 'absolute', left: 20, top: '50%', transform: 'translateY(-50%)', background: C.white, border: 'none', borderRadius: '50%', width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', zIndex: 10 }}>
                    <ChevronLeft size={22} color={C.forest} />
                  </button>
                  <button onClick={handleNextImage} style={{ position: 'absolute', right: 20, top: '50%', transform: 'translateY(-50%)', background: C.white, border: 'none', borderRadius: '50%', width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', zIndex: 10 }}>
                    <ChevronRight size={22} color={C.forest} />
                  </button>
                </>
              )}
            </div>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 20, justifyContent: 'center' }}>
                {images.map((img, i) => (
                  <button
                    ...
                    style={{
                      width: 72, height: 72, borderRadius: 8, overflow: 'hidden', border: i === currentImageIndex ? `2.5px solid ${C.forest}` : '2.5px solid transparent',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
            <div>
              ...
              <p style={{ fontFamily: FONTS.accent, fontSize: F_SIZE.lg, color: C.forest, fontWeight: 700, marginBottom: 20 }}>Optimal Formulation Edition</p>
               ...
               <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                 {(product.packages || []).map((pkg, i) => (
                   <button
                    ...
                    style={{
                      padding: '20px 24px', borderRadius: 8, border: i === selectedPackage ? `2.5px solid ${C.forest}` : '1.5px solid rgba(0,0,0,0.08)',
                      background: i === selectedPackage ? `${C.forest}04` : C.white,
                      cursor: 'pointer', transition: 'all 0.2s', textAlign: 'left',
                      position: 'relative'
                    }}
                   >
                     <div style={{ fontSize: F_SIZE.md, fontWeight: 900, color: i === selectedPackage ? C.forest : C.ink }}>{pkg.duration}</div>
                     ...
                     {pkg.savePct && (
                       <div style={{ 
                         ...
                         background: C.leaf, color: C.white, fontSize: 10, fontWeight: 900, borderRadius: 2
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 16 }}>
                <span style={{ fontSize: '2.5rem', fontWeight: 900, color: C.forest, letterSpacing: '-0.02em' }}>₹{price.toLocaleString()}</span>
              ...
              <div style={{ display: 'flex', gap: 20 }}>
                <div style={{ 
                  ...
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 28, padding: 0, color: C.forest, fontWeight: 800 }}>−</button>
                  <span style={{ fontSize: F_SIZE.lg, fontWeight: 900, minWidth: 24, textAlign: 'center', color: C.ink }}>{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 24, padding: 0, color: C.forest, fontWeight: 800 }}>+</button>
                </div>
                ...
                <motion.button
                  ...
                  style={{
                    flex: 1, background: C.forest, color: C.white, border: 'none',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 40, marginTop: 40 }}>
           ...
           <div style={{ 
             ...
             <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 32 }}>
               <div style={{ width: 48, height: 48, background: `${C.leaf}10`, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                 <Zap size={22} color={C.leaf} />
               </div>
             ...
             <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
               {(packageData.benefits || []).map((b: string, i: number) => (
                 <div key={i} style={{ display: 'flex', alignItems: 'start', gap: 16 }}>
                   <div style={{ width: 6, height: 6, borderRadius: '50%', background: C.leaf, marginTop: 8 }} />
             ...
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
               {(packageData.nutrients || []).map((n: ProductNutrient, i: number) => (
                 <div key={i} style={{ padding: '24px', borderRadius: 8, background: C.mist, border: '1px solid rgba(0,0,0,0.03)' }}>
                   ...
                   <div style={{ fontSize: F_SIZE.lg, fontWeight: 900, color: C.forest, lineHeight: 1 }}>{n.amount}</div>
        ...
        {showToast && (
          <motion.div
            ...
            style={{
              ...
              background: C.forest, color: C.white, padding: '16px 32px', borderRadius: 100,
